...
@Configuration
@EnableJms
public class RBSMessagingConfiguration {
    ...
    public JndiObjectFactoryBean jndiConnectionFactory(){
       ...
       // LOGGER.info("jndiConnectionFactory start"+jmsNamingProviderUrl);
        Properties props = new Properties();
        JndiTemplate jndiTemplate = new JndiTemplate();
        try {        	
        	props.put(Context.PROVIDER_URL, jmsNamingProviderUrl.trim());
			props.put(Context.SECURITY_PRINCIPAL, securityPrincipal.trim());
			props.put(Context.SECURITY_CREDENTIALS, securityCredentials.trim());
			props.put(Context.INITIAL_CONTEXT_FACTORY, javaNamingFactoryInitial.trim());
			props.setProperty("connectionFactory.ConnectionFactory",jmsNamingProviderUrl.trim());
    		jndiTemplate.setEnvironment(props);

    		JndiDestinationResolver jndiDestinationResolver = new JndiDestinationResolver();
    		jndiDestinationResolver.setJndiTemplate(jndiTemplate);

    		ConnectionFactory connectionFactory = (ConnectionFactory) jndiTemplate.lookup("jms/RemoteConnectionFactory");       	
			
			jndiObjectFactoryBean.setJndiTemplate(jndiTemplate);
		//	LOGGER.info("jndiConnectionFactory try block");
		} catch (NamingException e) {
			
			//LOGGER.info("jndiConnectionFactory exception block end");
		}
